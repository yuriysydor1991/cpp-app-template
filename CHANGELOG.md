...
## [appGtkmm4GLArea0.11.0] - 2026-08-17

- 2a0140c Introducing the InsertNewlineAtEOF clang-format option
- 8a08c81 Introducing the appCURLChatGPT branch info into the READMEs
- ebb0a08 Hiding the ApplicationContext fields and adding the atomic stop flag
- ece0045 Moving the linker hardening options out of the compile options
- 4032bce Split the DefaultLogger into a static proxy and a real logger instance
- 23e65b1 Introducing the appCURLClaude branch info into the READMEs
- 1d87a02 Introducing the external documentation URLs check into the Jenkins pipeline
- 8c7d6f6 Correcting the docker modules path of the documentation
- 507bc0a Introducing the quick build and the docker starter scripts
- af836e4 Introducing the quick build and the docker starter scripts
- d4566f3 Making the docker containers run under unprivileged users
- c936774 Formatting the sources with the clang-format
- edc68f4 Repairing the pipeline code check stages
- 7716e3b Introducing the appPGPLOT branch info into the READMEs
- fe8c4fc Fixing the Jenkisfile
- 40a41e2 Fix broken cppcheck link in section 5 index
- 003e092 Fix Groovy escape sequences in markdown link checker stage
- 3a93557 Rework markdown link checker stage to pure bash
- 98430c5 Add Jenkinsfile stage to detect broken markdown links; fix pre-existing broken links
- 10bddbd Reorganize doc/sections into section dirs with category grouping
- 64c4e7f Add an OpenSSL cmake enabler module with SHA-256, HMAC-SHA256 and HTTPS/TLS client code samples
- 1e323ed Group the enabler documentation under subsections in the READMEs
- afc9e85 Group the compression branches under a dedicated README section
- 3c0c58a Fix the misc/Jenkinsfile checkout branch to the appGtkmm4GLArea branch name
- 77365e1 Introducing the appLZMA branch info into the READMEs
- d348713 Add a liblzma (XZ Utils) compression library enabler module
- 1eb7748 Introducing the appZlib branch info into the READMEs
- 3ac54ce Add zlib compression usage examples to its enabler documentation
- 3ca6bce Replace the redundant PROJECT_NAME CMake variable with CMAKE_PROJECT_NAME
- 1f4dafd Introducing the appGtkmm4Vulkan branch info into the READMEs
- 60a983b Removing the appGtkmm4Vulkan branch info from the READMEs
- dbbbb00 Introducing the appQt6Vulkan and appGtkmm4Vulkan branches info into the READMEs
- e8822eb Simplifying root CMakeLists.txt
- d95b616 Introducing the appFirebird branch info into the READMEs
- d94170e Introducing the appGtkmm4GDBusClient branch info into the READMEs
- b22d721 Marking the appGtkmm4GLArea branch as current in the READMEs
- 31ff13a Introducing the appQt6GLArea branch info into the READMEs
- 82c575d Introducing the appGtkmm4GLArea branch info into the READMEs
- 6d6993b Introducing the GLArea widget workflow
