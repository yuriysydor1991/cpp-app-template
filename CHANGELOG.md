...
## [appCURL0.10.0] - 2026-08-17

- 63f96de Introducing the InsertNewlineAtEOF clang-format option
- 3ab38d6 Percent encoding the file protocol URLs of the CURLController test
- bce4841 Making the CURLController timeouts the constant long values
- d640424 Linking the libcurl into the CURLController object library
- 23ddb3f Failing the pipeline test stages when they match no tests
- c17d4ad Adding the missing default-logger tests subdirectory
- 9a72cba Introducing the appCURLChatGPT branch info into the READMEs
- 04cf3f3 Introducing the last_response_successfull call into the CURLController
- 40d3ef5 Introducing the post, the headers and the aliveness calls into the CURLController
- bbec63a Hiding the ApplicationContext fields and adding the atomic stop flag
- 4d3d5c3 Moving the linker hardening options out of the compile options
- 3dee17f Split the DefaultLogger into a static proxy and a real logger instance
- 28d56df Introducing the appCURLClaude branch info into the READMEs
