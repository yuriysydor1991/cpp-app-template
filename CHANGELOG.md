...
## [appGtkmm40.11.0] - 2026-08-17

- d2550fb Introducing the InsertNewlineAtEOF clang-format option
- ba8b0ad Introducing the appCURLChatGPT branch info into the READMEs
- 8bbd028 Hiding the ApplicationContext fields and adding the atomic stop flag
- 54c4e42 Moving the linker hardening options out of the compile options
- 707d59b Split the DefaultLogger into a static proxy and a real logger instance
- 40797b5 Introducing the appCURLClaude branch info into the READMEs
- 8943048 Introducing the external documentation URLs check into the Jenkins pipeline
- 88d8003 Correcting the docker modules path of the documentation
- 9901f28 Introducing the quick build and the docker starter scripts
- 7c2f6ec Introducing the quick build and the docker starter scripts
- f0c5c28 Making the docker containers run under unprivileged users
- 8f2b937 Formatting the sources with the clang-format
- 3b320fb Repairing the pipeline code check stages
- d9538ba Introducing the appPGPLOT branch info into the READMEs
- 97da0ad Fixing the Jenkinsfile
- fa01ad0 Fix broken cppcheck link in section 5 index
- b5e63e9 Fix Groovy escape sequences in markdown link checker stage
- 2bacd71 Rework markdown link checker stage to pure bash
- 5b332dc Add Jenkinsfile stage to detect broken markdown links; fix pre-existing broken links
- 85ff90d Reorganize doc/sections into section dirs with category grouping
- 7554286 Add an OpenSSL cmake enabler module with SHA-256, HMAC-SHA256 and HTTPS/TLS client code samples
- b184287 Group the enabler documentation under subsections in the READMEs
- c6058cd Group the compression branches under a dedicated README section
- 63875d8 Introducing the appLZMA branch info into the READMEs
- 4002dc6 Add a liblzma (XZ Utils) compression library enabler module
- 34e58d4 Introducing the appZlib branch info into the READMEs
- c4df756 Add zlib compression usage examples to its enabler documentation
- 01d634c Replace the redundant PROJECT_NAME CMake variable with CMAKE_PROJECT_NAME
- 7a48ff2 Introducing the appGtkmm4Vulkan branch info into the READMEs
- ee123e2 Removing the appGtkmm4Vulkan branch info from the READMEs
- 221e4aa Introducing the appQt6Vulkan and appGtkmm4Vulkan branches info into the READMEs
