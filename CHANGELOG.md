...
## [appPGPLOT0.11.0] - 2026-08-17

- d3734af Introducing the InsertNewlineAtEOF clang-format option
- ce64704 Introducing the appCURLChatGPT branch info into the READMEs
- 6c38e42 Hiding the ApplicationContext fields and adding the atomic stop flag
- 75cf93d Moving the linker hardening options out of the compile options
- c1d71ea Split the DefaultLogger into a static proxy and a real logger instance
- b845f25 Introducing the appCURLClaude branch info into the READMEs
- 556ee76 Introducing the external documentation URLs check into the Jenkins pipeline
- 0714398 Correcting the docker modules path of the documentation
- 1b00062 Introducing the quick build and the docker starter scripts
- 199b090 Introducing the quick build and the docker starter scripts
- 1adff81 Making the docker containers run under unprivileged users
- dc12bd1 Formatting the sources with the clang-format
- 3258d18 Repairing the pipeline code check stages
- 4a8020b Integrating and implementing the basic PGPLOT library example
- 85ce225 Fixing the Jenkinsfile
- 7a6b12f Fix broken cppcheck link in section 5 index
- 21c6431 Fix Groovy escape sequences in markdown link checker stage
- 042c9fa Rework markdown link checker stage to pure bash
- 6f2e825 Add Jenkinsfile stage to detect broken markdown links; fix pre-existing broken links
- 323c34a Reorganize doc/sections into section dirs with category grouping
- fe506e0 Add an OpenSSL cmake enabler module with SHA-256, HMAC-SHA256 and HTTPS/TLS client code samples
- d7d1ce9 Group the enabler documentation under subsections in the READMEs
- dbc8107 Group the compression branches under a dedicated README section
- ab1a000 Introducing the appLZMA branch info into the READMEs
