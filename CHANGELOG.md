...
## [appQt6GLArea0.10.0] - 2026-08-17

- 002dca1 Introducing the InsertNewlineAtEOF clang-format option
- 42373ac Introducing the appCURLChatGPT branch info into the READMEs
- c4b9bf9 Adding the missing PRIVATE keyword into the real default logger linking
- 375ef12 Hiding the ApplicationContext fields and adding the atomic stop flag
- 1ccdd48 Moving the linker hardening options out of the compile options
- 95a0c51 Split the DefaultLogger into a static proxy and a real logger instance
- 8cda2ec Introducing the appCURLClaude branch info into the READMEs
- 536d0e3 Marking the appQt6GLArea branch as the current one in the READMEs
- a5633f2 Introducing the raw OpenGL rendering item into the Qt6 QML window
- d073c02 Switching the pipeline checkout onto the appQt6GLArea branch
- 8f41af2 Introducing the external documentation URLs check into the Jenkins pipeline
- 20c5bb9 Correcting the docker modules path of the documentation
- 453b1f0 Introducing the quick build and the docker starter scripts
- 1481bba Introducing the quick build and the docker starter scripts
- 91107c3 Making the docker containers run under unprivileged users
- 28aaa07 Formatting the sources with the clang-format
- 361f9de Repairing the pipeline code check stages
- ef6fe2b Introducing the appPGPLOT branch info into the READMEs
- e6f196a Fixing the Jenkinsfile
- 98a8c69 Fix broken cppcheck link in section 5 index
- 3268a65 Fix Groovy escape sequences in markdown link checker stage
- f8e37de Rework markdown link checker stage to pure bash
- 2462523 Add Jenkinsfile stage to detect broken markdown links; fix pre-existing broken links
- f042ddb Reorganize doc/sections into section dirs with category grouping
- 87e404e Add an OpenSSL cmake enabler module with SHA-256, HMAC-SHA256 and HTTPS/TLS client code samples
- 09e8a07 Group the enabler documentation under subsections in the READMEs
- d4fb1f1 Group the compression branches under a dedicated README section
- e32ba82 Introducing the appLZMA branch info into the READMEs
- df4986c Add a liblzma (XZ Utils) compression library enabler module
- e4e5641 Introducing the appZlib branch info into the READMEs
- cf63806 Add zlib compression usage examples to its enabler documentation
- 129bad1 Replace the redundant PROJECT_NAME CMake variable with CMAKE_PROJECT_NAME
- 740bec4 Introducing the appGtkmm4Vulkan branch info into the READMEs
- 8d62f99 Removing the appGtkmm4Vulkan branch info from the READMEs
- f0b9185 Introducing the appQt6Vulkan and appGtkmm4Vulkan branches info into the READMEs
- c557212 Simplifying root CMakeLists.txt
- 4c9f7d3 Introducing the appFirebird branch info into the READMEs
- a083f74 Introducing the appGtkmm4GDBusClient branch info into the READMEs
- 9851eaf Introducing the appQt6GLArea branch info into the READMEs
- 311219d Introducing the appGtkmm4GLArea branch info into the READMEs
- 7464d85 Splitting the Data Visualization README category into Plots/Graphs and Maps
- 8474be4 Introducing the applibMeson branch info into the READMEs
- 104b019 Introducing the zlib and image format cmake enablers with docs
- 14061df Introducing the appSFML branch info into the READMEs
- ec65ca9 Introducing the appwxWidgets branch info into the READMEs
- fb22910 Introducing the appQt6QtDBusClient branch info into the READMEs
