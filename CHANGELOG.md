...
## [appQt6LeafLet0.10.0] - 2026-08-17

- 287899b Introducing the InsertNewlineAtEOF clang-format option
- 430ea2b Introducing the appCURLChatGPT branch info into the READMEs
- 2e46a57 Hiding the ApplicationContext fields and adding the atomic stop flag
- a86e6c4 Moving the linker hardening options out of the compile options
- 8b83738 Adding the missing keyword to the real default logger link command
- 4123b99 Split the DefaultLogger into a static proxy and a real logger instance
- 652209c Introducing the appCURLClaude branch info into the READMEs
- 96f5348 Introducing the external documentation URLs check into the Jenkins pipeline
- 65450ee Correcting the docker modules path of the documentation
- 8725af1 Introducing the quick build and the docker starter scripts
- f17630f Introducing the quick build and the docker starter scripts
- 633af29 Making the docker containers run under unprivileged users
- e4e75f0 Formatting the sources with the clang-format
- 3224b74 Repairing the pipeline code check stages
- ff802e6 Introducing the appPGPLOT branch info into the READMEs
- 3e3f328 Fixing the Jenkinsfile
- 6b9a570 Fix broken cppcheck link in section 5 index
- ca77542 Fix Groovy escape sequences in markdown link checker stage
- e5b901e Rework markdown link checker stage to pure bash
- 7497e32 Add Jenkinsfile stage to detect broken markdown links; fix pre-existing broken links
- 7edb87b Reorganize doc/sections into section dirs with category grouping
- 14bfb25 Add an OpenSSL cmake enabler module with SHA-256, HMAC-SHA256 and HTTPS/TLS client code samples
- 6b04721 Group the enabler documentation under subsections in the READMEs
- ba1d5a4 Group the compression branches under a dedicated README section
- b804abf Fix the misc/Jenkinsfile checkout branch to the appQt6LeafLet branch name
- 94ad3ae Introducing the appLZMA branch info into the READMEs
- 076b839 Add a liblzma (XZ Utils) compression library enabler module
- 6d70364 Introducing the appZlib branch info into the READMEs
- 0c4564f Add zlib compression usage examples to its enabler documentation
- fd8453a Replace the redundant PROJECT_NAME CMake variable with CMAKE_PROJECT_NAME
- 3ae02ba Introducing the appGtkmm4Vulkan branch info into the READMEs
- a54c576 Removing the appGtkmm4Vulkan branch info from the READMEs
- f219a71 Introducing the appQt6Vulkan and appGtkmm4Vulkan branches info into the READMEs
- ed51afb Simplifying root CMakeLists.txt
- 8de9c7f Introducing the appFirebird branch info into the READMEs
- 21737cd Introducing the appGtkmm4GDBusClient branch info into the READMEs
- d991aab Introducing the appQt6GLArea branch info into the READMEs
- 60349a2 Introducing the appGtkmm4GLArea branch info into the READMEs
- e10f113 Splitting the Data Visualization README category into Plots/Graphs and Maps
- c08af0b Introducing the applibMeson branch info into the READMEs
- 12e6ddd Introducing the zlib and image format cmake enablers with docs
- 3571de7 Introducing the appSFML branch info into the READMEs
- ec6fb89 Introducing the appwxWidgets branch info into the READMEs
- 5fb63ca Introducing the appQt6QtDBusClient branch info into the READMEs
- 3641b3a Introducing the appQt6QtLocation branch info into the READMEs
- 2e2e3f7 Introducing the appQt6ChartView branch into the READMEs
- 8f83a45 Marking appQt6LeafLet as a current branch
- 45451ff Introducing the appGtkmm4LeafLet and appQt6LeafLet branches info into the READMEs
- 0cd58a8 Introducing the Leaflet map QML component instead of DefaultWelcome
