...
- `appCocoa` branch at [[GitHub](https://github.com/yuriysydor1991/cpp-app-template/tree/appCocoa), [GitLab](https://gitlab.com/yuriysydor1991/cpp-app-template/tree/appCocoa)] (**current**) for the application general classes with additional defined structure for native macOS [Cocoa](https://developer.apple.com/documentation/appkit) / AppKit desktop application development in [Objective-C++](https://en.wikipedia.org/wiki/Objective-C%2B%2B). This is a **macOS-only** branch: the GNU/Linux packagers (flatpak, snap, deb, rpm, FreeBSD pkg), the Windows WIX (MSI) packager, the Docker integration and the Valgrind/gprof profilers were removed, macOS .dmg/.pkg packaging was added, and the Jenkins pipeline targets a macOS agent. The platform-independent core still builds and is unit/component tested on any platform.
...
1. [Project structure](/doc/sections/en_US/4-project-structure/4-project-structure.md)
    ...
    1. [Cocoa (AppKit) + Objective-C++ integration](/doc/sections/en_US/4-project-structure/4-8-cocoa-objcpp-integration.md)
