...
# (see the OpenGameArtSound::alias accessor) and, at the same time, the path of it's
# sound file relative to the OPENGAMEART_AUDIO_ROOT_DIR directory.
#
#   template_project_opengameart_audio_select_sounds(
...
function(template_project_opengameart_audio_select_sounds)
  ...
  if (NOT ARG_ALIASES_VAR)
    message(FATAL_ERROR "No ALIASES_VAR output variable given to select the OpenGameArt sounds into")
  endif()

  if (NOT ARG_PACKS AND NOT ARG_SOUNDS)
    set(ARG_PACKS ${OPENGAMEART_AUDIO_PACKS})
  endif()
  ...
  foreach(pack IN LISTS ARG_PACKS)
    ...
    if (NOT packFiles)
      message(FATAL_ERROR "No '${pack}' pack sounds inside the ${OPENGAMEART_AUDIO_ROOT_DIR} directory")
    endif()
  ...
  foreach(sound IN LISTS ARG_SOUNDS)
    if (NOT EXISTS "${OPENGAMEART_AUDIO_ROOT_DIR}/${sound}")
      message(FATAL_ERROR "No '${sound}' sound inside the ${OPENGAMEART_AUDIO_ROOT_DIR} directory")
    endif()
...
function(template_project_opengameart_audio_write_manifest outputFile content)
...
# their "<pack>/<path>" aliases, so the OpenGameArtQtResourcePath class resolves
# them at the run time.
#
#   template_project_opengameart_audio_write_qrc(
#     <output-file>
#     [PREFIX <resource-prefix>]  # OPENGAMEART_AUDIO_QT_RESOURCE_PREFIX by default
#     [PACKS  <pack>...]
#     [SOUNDS <pack>/<path>...]
#   )
function(template_project_opengameart_audio_write_qrc outputFile)
  ...
  if (NOT ARG_PREFIX)
    set(ARG_PREFIX "${OPENGAMEART_AUDIO_QT_RESOURCE_PREFIX}")
  endif()
  ...
  foreach(alias IN LISTS aliases)
    list(APPEND entries "    <file alias=\"${alias}\">${OPENGAMEART_AUDIO_ROOT_DIR}/${alias}</file>")
  endforeach()

  string(JOIN "\n" entriesText ${entries})

  template_project_opengameart_audio_write_manifest(
    "${outputFile}"
    "<!DOCTYPE RCC><RCC version=\"1.0\">\n  <qresource prefix=\"${ARG_PREFIX}\">\n${entriesText}\n  </qresource>\n</RCC>\n")

  list(LENGTH aliases count)

  message(STATUS "OpenGameArt Qt resource manifest with ${count} sound(s): ${outputFile}")
endfunction()
...
# "<pack>/<path>" aliases, so the OpenGameArtGResourcePath class resolves them at
# the run time.
#
...
#     <output-file>
#     [PREFIX <resource-prefix>]  # OPENGAMEART_AUDIO_GRESOURCE_PREFIX by default
#     [COMPRESSED]
#     [PACKS  <pack>...]
#     [SOUNDS <pack>/<path>...]
#   )
function(template_project_opengameart_audio_write_gresource outputFile)
  ...
  if (NOT ARG_PREFIX)
    set(ARG_PREFIX "${OPENGAMEART_AUDIO_GRESOURCE_PREFIX}")
  endif()
  ...
  foreach(alias IN LISTS aliases)
    list(APPEND entries "    <file alias=\"${alias}\"${compressedAttribute}>${OPENGAMEART_AUDIO_ROOT_DIR}/${alias}</file>")
  endforeach()
  ...
  message(STATUS "OpenGameArt GResource manifest with ${count} sound(s): ${outputFile}")
endfunction()
