...
Щоб одразу приступити до реалізації конкретної програми необхідно перейти до файлу `GtkmmIniter.cpp` і/або `GtkmmIniter.h` відповідного класу, який призначений для утримання початкового коду реалізації програми. Візуальний інтерфейс може розроблятись у програмі [Glade](https://en.wikipedia.org/wiki/Glade_Interface_Designer) відкриваючи файл `src/gtkmm3/ui/GtkmmWindow.glade`. Даний файл відповідальний за композицію UI автоматично компілюється у заголовковий файл C++ під час конфігурування проекту за допомогою програми `cmake`.
