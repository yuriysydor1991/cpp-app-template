...
To proceed the application implementation right away look for the `Application` class' `src/app/applications/Application.cpp` file which is designed to accept initial code of the application. Specifically, new code may be placed into the `int Application::run(std::shared_ptr<ApplicationContext> ctx)` method.
