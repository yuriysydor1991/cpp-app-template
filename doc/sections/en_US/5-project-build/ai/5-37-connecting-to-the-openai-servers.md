...
The `curli::CURLController` class performs the request with it's `post` method, which sends a body with a set of headers and keeps the HTTP status code of the answer available through the `last_response_code` method. Unlike it's `download` method, the `post` one applies no low speed abort and waits far longer, because a model composing an answer sends nothing over the connection meanwhile. See the [Enabling the libcurl](/doc/sections/en_US/5-project-build/5-14-enabling-libcurl.md) section for the rest of that class.
