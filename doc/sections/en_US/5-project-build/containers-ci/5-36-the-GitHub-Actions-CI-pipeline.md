...
The runner is a GNU/Linux one, so the jobs cover the platform independent core of the branch: its macOS specific part stays out of the build, the way it does on any non-Apple host, and the macOS agent of the [misc/Jenkinsfile](/misc/Jenkinsfile) pipeline is the one building it.
