...
| `build-and-test` | configures the Debug build with the sanitizers, the unit, the component and the integration tests, builds it and runs the `UTEST_`, the `CTEST_` and the `ITEST_` `ctest` suites |
