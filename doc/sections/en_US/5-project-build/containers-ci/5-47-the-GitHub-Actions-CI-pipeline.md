...
| `clang-tidy` | configures with `-DENABLE_CLANG_TIDY=ON` and builds, so the checks run during the compilation - a manual run with the parameter below is the only one starting the job |
...
| `run_clang_tidy` | `RUN_CLANGTIDY` | the whole clang-tidy job, which a push and a pull request skip |

Both stay off by default, since the checks cost minutes of the runner time which every push would pay for.
