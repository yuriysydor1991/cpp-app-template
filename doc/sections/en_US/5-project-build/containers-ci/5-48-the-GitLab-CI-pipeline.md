...
| `clang-tidy` | configures with `-DENABLE_CLANG_TIDY=ON` and builds, so the checks run during the compilation - a manual run with the variable below is the only one starting the job |
...
variables:
  ...
  DEPENDENCIES: libsdl2-dev libsdl2-image-dev libsdl2-ttf-dev libsdl2-net-dev libglu1-mesa-dev
```
...
| `RUN_CLANG_TIDY` | `RUN_CLANGTIDY` | the whole `clang-tidy` job, which a push and a merge request skip |

Both stay `false` by default, since the checks cost minutes of the runner time which every push would pay for.
