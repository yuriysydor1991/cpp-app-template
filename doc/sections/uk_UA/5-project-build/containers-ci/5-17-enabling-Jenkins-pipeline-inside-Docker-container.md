...
`flatpak-builder` виконує побудову всередині пісочниці [bubblewrap](https://github.com/containers/bubblewrap), а середовище виконання AppImage монтує себе через FUSE, що заборонено типовими профілями контейнера, отож ціль `jenkins-pipeline-docker-run` запускає контейнер із пристроєм `/dev/fuse`, можливістю `SYS_ADMIN` та незабороняючими профілями seccomp і AppArmor. Пісочниця bubblewrap монтує власну procfs, що ядро забороняє, допоки приховані та доступні лише для читання шляхи `/proc` контейнера тримають вже змонтовану частково прихованою, отож контейнер отримує ще й незаборонені системні шляхи.
