...
| `clang-tidy` | конфігурує з `-DENABLE_CLANG_TIDY=ON` і будує, отож перевірки виконуються під час компіляції - завдання запускається лише ручним запуском із наведеною нижче змінною |
...
| `RUN_CLANG_TIDY` | `RUN_CLANGTIDY` | усе завдання `clang-tidy`, яке push та merge request оминають |

Типово обидві лишаються `false`, оскільки перевірки коштують хвилин часу виконавця, за які платив би кожен push.
