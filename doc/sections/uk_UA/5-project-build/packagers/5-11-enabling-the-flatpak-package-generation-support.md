...
mkdir -vp build && cd build && cmake ../ -DENABLE_FLATPAK=ON -DENABLE_GTKMM4=OFF
```

Успішно виконана команда у свою чергу увімкне ціль для побудови під назвою `flatpak`.

Програма `flatpak-builder` конфігурує і будує проект з вихідних кодів усередині власного пісочного середовища, де використовуються стек gtkmm4 та WebKitGTK з SDK, тож дерево побудови на хості лише генерує ціль `flatpak`. Параметр `ENABLE_GTKMM4=OFF` звільняє це дерево від вимоги наявності пакетів розробника gtkmm4 і WebKitGTK, тож його необхідно прибрати якщо те саме дерево побудови має будувати проект і на самому хості.
