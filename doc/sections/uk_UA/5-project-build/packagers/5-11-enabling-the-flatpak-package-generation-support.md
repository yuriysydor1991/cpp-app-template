...
У разі успішної побудови цілі буде створено файл під назвою `CppAppTemplate-0.12.0.flatpak` (версія може змінюватись) у кореневій директорії побудови проекту шаблону. Для того щоб преглянути або/і змінити ціль побудови пакету flatpak необхідно відвідати файли `cmake/template-project-flatpak-target.cmake` або `misc/packagers/flatpak.conf.json.in`.
