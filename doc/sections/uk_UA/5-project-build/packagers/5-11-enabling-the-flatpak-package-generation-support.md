...
Програма `flatpak-builder` збирає проект усередині власного оточення, котре не має доступу до мережі, тоді як вмикач OpenGameArt звантажує свої пакунки звуків через Інтернет. Тому згенерований маніфест передає модулю, як власне джерело, ту директорію пакунків, котру наповнило конфігурування на хості — звантажену, або ж ту, на яку вказала змінна `TEMPLATE_APP_OPENGAMEART_AUDIO_DIR`, — а конфігурування всередині оточення дістає її через ту саму змінну. Запакованій програмі надається `--socket=pulseaudio`, аби програвач звуку справді дістався звукового пристрою.
