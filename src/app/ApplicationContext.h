...
class ApplicationContext
{
 public:
  ...
  const std::string& get_openai_token() const;

  /// @brief Sets the the OpenAI API token to authorize the ChatGPT calls with.
  /// When empty the OAIController takes the token from the environment variable
  /// named in it's TOKEN_ENV_VAR field.
  ...
  const std::string& get_openai_question() const;

  /// @brief Sets the the question to ask the ChatGPT about. When empty the
  /// Application::run only prints the hint on how to provide one.
  ...
  const std::string& get_openai_model() const;

  /// @brief Sets the an optional chat completion model override. When empty the
  /// OAIController asks it's DEFAULT_MODEL one.
