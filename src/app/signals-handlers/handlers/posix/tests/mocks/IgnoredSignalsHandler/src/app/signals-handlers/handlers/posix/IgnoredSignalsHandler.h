...
class IgnoredSignalsHandler : public ISignalsHandler
{
 public:
  ~IgnoredSignalsHandler() override = default;

  IgnoredSignalsHandler()
  {
    if (onMockCreate) {
      onMockCreate(*this);
    }
  ...
  inline static std::function<void(IgnoredSignalsHandler&)> onMockCreate;
