...
class UTEST_Application : public Test
{
 public:
  ...
  ~UTEST_Application() { CocoaIniter::onMockCreate = nullptr; }
...
{
  MockFunction<void(CocoaIniter & instance)> initerEnsurer;

  EXPECT_CALL(initerEnsurer, Call(_))
      .Times(1)
      .WillOnce(Invoke([&](CocoaIniter& instance) {
        EXPECT_CALL(instance, run(appCtx)).Times(1).WillOnce(Return(0));
      }));

  CocoaIniter::onMockCreate = initerEnsurer.AsStdFunction();
