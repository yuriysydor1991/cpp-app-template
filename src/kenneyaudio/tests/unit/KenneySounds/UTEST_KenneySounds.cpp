...
class UTEST_KenneySounds : public Test
{
 public:
...
TEST_F(UTEST_KenneySounds, gives_the_sounds_of_a_single_pack)
{
...
TEST_F(UTEST_KenneySounds, gives_no_sounds_of_an_unknown_pack)
{
  EXPECT_TRUE(sounds->ofPack("no-such-pack").empty());
}

TEST_F(UTEST_KenneySounds, finds_a_sound_by_it_s_pack_and_path)
{
...
TEST_F(UTEST_KenneySounds, gives_no_sound_of_an_unknown_path)
{
  EXPECT_EQ(sounds->find("interface-sounds", "Audio/no_such.ogg"), nullptr);
}

TEST_F(UTEST_KenneySounds, searches_the_sounds_by_a_name_part)
{
...
TEST_F(UTEST_KenneySounds, a_search_reaches_every_pack)
{
  EXPECT_EQ(sounds->search("_").size(), sounds->count());
}

TEST_F(UTEST_KenneySounds, an_unmatched_search_gives_nothing)
{
  EXPECT_TRUE(sounds->search("no-such-sound").empty());
}

TEST_F(UTEST_KenneySounds, a_set_with_no_packs_stays_empty)
{
  auto empty = KenneySounds::create(nullptr);
