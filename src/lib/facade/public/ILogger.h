 ...
 * LOG_INIT_REAL_LOGGER macro, to make the whole binary log through it. The
 * proxy itself is not installed, because every derived library owns a copy of
 * it, so the public LibraryFacade::init_logger is the only name a library user
 * needs.
 *
 * Current file lives among the installable public headers, because that method
 * accepts this interface and the library user implements it. Unlike the rest of
 * them it deliberately carries neither the version carrying library namespace
 * nor the matching include guard, and it is not marked with the
 * TEMPLATE_LIB_API macro either. It is the single logging contract every
 * library derived from this template accepts, so one application logger
 * implementation serves all of the derived libraries an application depends on
 * at once, and the first installed copy of this header which an application
 * includes stands for all of them. The calls into an implementation of it are
 * plain virtual dispatch through the object own virtual table, so nothing of it
 * has to leave a shared object.
 *
