#ifndef YOUR_CPP_APP_TEMPLATE_PROJECT_OPENGAMEARTSDLMIXERSOUNDPLAYER_CLASS_H
#define YOUR_CPP_APP_TEMPLATE_PROJECT_OPENGAMEARTSDLMIXERSOUNDPLAYER_CLASS_H
 ...
 * the .ogg and the .mp3 submissions OpenGameArt hosts among them - come from
 * the SDL_mixer satellite library. Which of it's decoders the library at hand
 * really carries is a build decision of that library, hence the extensions
 * answered for are the ones it really brought up rather than a hand written
 * list.
 */
class OpenGameArtSdlMixerSoundPlayer : public IOpenGameArtSoundPlayer
{
 public:
  /**
   ...
   * mixing device up.
   */
  OpenGameArtSdlMixerSoundPlayer();

  /**
   * @brief Takes them all back down.
   */
  ~OpenGameArtSdlMixerSoundPlayer() override;
...
#endif  // YOUR_CPP_APP_TEMPLATE_PROJECT_OPENGAMEARTSDLMIXERSOUNDPLAYER_CLASS_H
